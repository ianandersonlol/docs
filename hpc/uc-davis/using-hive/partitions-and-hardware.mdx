...
<Tabs>
  ...
  <Tab title="GPU Partitions">
    **Use `gpu-a100` for:**
    ...
    - Memory-intensive GPU workloads (80 GB HBM2e per GPU)
...
|----------|----------|--------|-----------|----------|
| NVIDIA A100 | 8 (4 per node × 2 nodes) | 80 GB HBM2e | gpu-a100 | Deep learning training, large models |
...
# Request any available GPU in the selected partition
sbatch -p gpu-a100 --gres=gpu:1 job.sh
```

<Note>
  HIVE separates A100 and A6000 GPUs into different partitions. Choose `gpu-a100` or `gpu-a6000` based on the GPU family you want.
</Note>
