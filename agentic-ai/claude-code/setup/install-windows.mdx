...
<Warning>
  **The free Claude plan does not include Claude Code in the desktop app.** For this guide, use **Pro**, **Max**, **Team**, or **Enterprise**. If the Code features won't work after you sign in, this is almost always the reason.
</Warning>
...
Here's the whole Windows install at a glance, across the two sections below:

```mermaid
graph TD
    A[Download the app] --> B[Install Git for Windows]
    B --> C[Restart the app]
    C --> D[Sign in]
    D --> E[Find the Code tab]
```
...
If you read anything online about Claude Code on Windows, you'll hit the term **[WSL](/agentic-ai/claude-code/glossary)** (Windows Subsystem for Linux). People treat it like a required step. For your situation, it usually isn't. Here's the plain-English version.
